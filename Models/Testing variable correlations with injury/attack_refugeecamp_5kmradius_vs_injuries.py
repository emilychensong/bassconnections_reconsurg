...
def within_1km_radius(attack_coords):
...
attacks_df['within_1km'] = attacks_df.apply(
    lambda row: within_1km_radius((row['latitude'], row['longitude'])), axis=1
)

# Step 3: Count the number of "yes" occurrences for each date
attack_counts_by_date = attacks_df.groupby('date')['within_1km'].apply(lambda x: (x == 'yes').sum()).reset_index()
attack_counts_by_date.columns = ['date', 'within_1km_count']

# Step 4: Merge with injury data
merged_df = pd.merge(attack_counts_by_date, injuries_df, on='date', how='inner')

# Check for missing values and drop them
merged_df.dropna(subset=['within_1km_count', 'number of injuries'], inplace=True)
...
X = filtered_df['within_1km_count']  # Independent variable: count of attacks within 5km
...
plt.scatter(filtered_df['within_1km_count'], filtered_df['number of injuries'], color='blue', label='Data Points')

# Plotting the regression line
predicted_y = model.predict(X)
plt.plot(filtered_df['within_1km_count'], predicted_y, color='red', label='Regression Line', linewidth=2)

# Adding titles and labels
plt.title('Linear Regression: Count of Attacks within 1 km vs. Number of Injuries (Outliers Removed)')
plt.xlabel('Count of Attacks within 1 km')
...
#%% COUNTS OF ATTACK TYPES THAT WERE IN 1 KM RADIUS
...
attacks_within_1km = attacks_df[attacks_df['within_1km'] == 'yes']
print("Sample of attacks within 1 km radius:")
print(attacks_within_1km.head())

# Step 2: Map each attack to its broader category
attacks_within_1km['attack_category'] = attacks_within_1km['sub_event_type'].map(sub_event_type_mapping)
print("Sample after mapping event types to attack categories:")
print(attacks_within_1km[['event_type', 'attack_category']].head())

# Step 3: Count occurrences of each attack category
attack_type_counts = attacks_within_1km['attack_category'].value_counts()
print("Attack type counts within 1 km radius:")
...
# Adding titles and labels
plt.title('Count of Attack Types within 1 km Radius of Refugee Camps')
...
attack_type_counts_within_1km = attacks_within_1km['attack_category'].value_counts()

# Step 3: Calculate proportion of each attack type within the 5 km radius
attack_type_proportions = (attack_type_counts_within_1km / total_attack_counts).fillna(0)  # Fill NaN with 0 if no attacks of that type

print("Proportion of each attack type within 1 km radius:")
...
# Adding titles and labels
plt.title('Proportion of Attack Types within 1 km Radius of Refugee Camps')
plt.xlabel('Attack Type')
plt.ylabel('Proportion of Attacks within 1 km')
plt.xticks(rotation=45)
plt.grid(axis='y')
...
total_yes_attacks = attack_type_counts_within_1km.sum()

# Step 2: Calculate the proportion of each attack type relative to the total "yes" attacks
attack_type_proportions_within_1km = attack_type_counts_within_1km / total_yes_attacks

# Step 3: Plot the pie chart
plt.figure(figsize=(12, 12))
attack_type_proportions_within_1km.plot(
    kind='pie',
...
#%% ABOVE BUT AS BAR GRAPH

# Step 1: Calculate the total number of "yes" attacks within the 1 km radius
total_yes_attacks = attack_type_counts_within_1km.sum()
...
plt.xlabel('Attack Type')
plt.ylabel('Proportion of Total "Yes" Attacks')
plt.xticks(rotation=45)
plt.grid(axis='y')
