...
Independent Variables: # of attacks (up to Oct 11 24), event_type (up to Oct 11 24)
...
merged_data = pd.merge(attacks_injuries, acled_data[['event_date', 'event_type']], left_on='date', right_on='event_date')

# Drop any rows with missing values in the key columns
clean_data = merged_data.dropna(subset=['number_of_attacks', 'number_of_injuries', 'event_type'])
...
clean_data_encoded = pd.get_dummies(clean_data, columns=['event_type'], drop_first=True)
...
formula = 'number_of_injuries ~ number_of_attacks + ' + ' + '.join(
    [col for col in clean_data_encoded.columns if col.startswith('event_type_')]
)
...
pred_data = pd.DataFrame({
    ...
    'event_type_Violence_against_civilians': y.ravel()
})

# Ensure all other sub_event_type columns are zero in the prediction data
for col in clean_data_encoded.columns:
    if col.startswith('event_type_') and col != 'event_type_Violence_against_civilians':
        pred_data[col] = 0
...
ax.scatter(
    ...
    clean_data_encoded['event_type_Violence_against_civilians'], 
...
ax.set_ylabel('Attack Type (Violence against civilians)')
...
formula = 'number_of_injuries ~ number_of_attacks + ' + ' + '.join(
    [col for col in train_data.columns if col.startswith('event_type_')]
)
